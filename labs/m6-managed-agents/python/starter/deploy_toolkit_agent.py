"""deploy_toolkit_agent — Module 6 lab: the Codebase Toolkit's bug-hunter as a Claude Managed Agent (STARTER — fill the TODO(step-n) blocks; solution next door in ../solution/).
...
def step1(state: dict, **_) -> None:
    if "environment_id" in state:
        ...
        return
    # TODO(step-1): env = client().beta.environments.create(name=f"ws-{USER}", config={...})
    #   config: type "cloud"; packages.pip ["ruff"]; networking type "limited" with allowed_hosts
    #   github.com, api.github.com, raw.githubusercontent.com; allow_package_managers True; allow_mcp_servers False
    raise NotImplementedError("TODO(step-1): create the environment (see module 6 §10.1 (1))")
...
def agent_tools() -> list[dict]:
    # TODO(step-2): return the two tools[] entries from module 6 §10.1 (2):
    ...
    raise NotImplementedError("TODO(step-2): agent tools")
...
def step2(state: dict, **_) -> None:
    ...
    if "agent_id" in state:
        ...
        # TODO(step-2): agent = c.beta.agents.create(name=f"codebase-toolkit-{USER}", model=MODEL, system=SYSTEM,
        #                                            tools=agent_tools(), metadata={"workshop": "claude-builders-v4", "owner": USER})
        agent = None
        raise NotImplementedError("TODO(step-2): create the agent, print agent.id and agent.version (expect 1)")
    ...
    if (agent.system or "").endswith(EXTRA_LINE.strip()):
        ...
        try:
            # TODO(step-2): agent = c.beta.agents.update(agent.id, version=agent.version, system=(agent.system or SYSTEM) + EXTRA_LINE)
            raise NotImplementedError("TODO(step-2): update the agent's system prompt -> version 2")
...
def resolve(session_id: str, pending: list, auto_yes: bool) -> None:
    ...
    for ev in pending:
        # TODO(step-3):
        #   if ev.type == "agent.custom_tool_use": result = create_ticket(**ev.input); reply =
        #       {"type": "user.custom_tool_result", "custom_tool_use_id": ev.id, "content": [{"type": "text", "text": str(result)}]}
        #   else (agent.tool_use with evaluated_permission "ask"): ask the human unless auto_yes; reply =
        #       {"type": "user.tool_confirmation", "tool_use_id": ev.id, "result": "allow" | "deny", "deny_message"?: str}
        #   client().beta.sessions.events.send(session_id, events=[reply])
        raise NotImplementedError(f"TODO(step-3): answer pending event {ev.id} ({ev.type})")
...
def stream_turn(session_id: str, *, send: list | None = None, replay_history: bool = False,
    ...
    idles = 0
    last_reason = None
    # TODO(step-3): this is the no-race pattern — keep the ORDER when you fill it in:
    #   1) open  c.beta.sessions.events.stream(session_id)          (as a context manager)
    #   2) if replay_history: backlog = list(c.beta.sessions.events.list(session_id))
    #   3) if send: c.beta.sessions.events.send(session_id, events=send)
    #   then iterate itertools.chain(backlog, stream), skip event_start/event_delta and already-seen ids, render(ev),
    #   and on session.status_idle: requires_action -> resolve(...) and continue; anything else -> print and break.
...
def step3(state: dict, *, auto_yes: bool = False, new_session: bool = False, **_) -> None:
    ...
    if "session_id" in state and not new_session:
        ...
        return
    # TODO(step-3): session = c.beta.sessions.create(agent=agent_id, environment_id=env_id,
    #                   title=f"paymentservice bug hunt ({USER})", metadata={...}, initial_events=[user_message(TASK)])
    session = None
    raise NotImplementedError("TODO(step-3): create the session with initial_events")
...
def step5(state: dict, **_) -> None:
    ...
    sid = need(state, "session_id", "step3")
    found = False
    # TODO(step-5): for f in c.beta.files.list(scope_id=sid, betas=[BETA]): print(f.id, f.filename);
    #               download the one ending in bug-report.md with c.beta.files.download(f.id).write_to_file("bug-report.md")
    for f in []:
        found = bool(f)
