/* deploy_toolkit_agent.ts — Module 6 TypeScript track (SOLUTION).
  ...
  if (state.agent_id) { console.log(`reusing ${state.agent_id}`); agent = await client.beta.agents.retrieve(state.agent_id); }
    ...
    agent = await client.beta.agents.create({
      ...
      tools: [
        ...
        { type: "custom", name: "create_ticket", description: CREATE_TICKET_DESCRIPTION,
          input_schema: CREATE_TICKET_INPUT_SCHEMA as any },
      ] as any,
    });
    ...
    if (ev.type === "agent.message") process.stdout.write(ev.content.map((b: any) => (b.type === "text" ? b.text : "")).join(""));
      ...
      for (const id of ev.stop_reason.event_ids as string[]) {                 // answer EVERY pending id
        ...
        if (pending.type === "agent.custom_tool_use") {
          const result = pending.name === "create_ticket" ? await createTicket(pending.input) : `unknown tool ${pending.name}`;
          ...
          const ok = AUTO_YES || (await rl.question(`\nAllow ${pending.name} ${JSON.stringify(pending.input).slice(0, 100)}? [a/d] `)).trim().toLowerCase().startsWith("a");
...
async function step3(state: State) {
  ...
  const session = await client.beta.sessions.create({ agent: agentId, environment_id: envId, title: `paymentservice bug hunt (${USER})` });
...
async function step5(state: State) {
  const sid = need(state, "session_id", "step3");
  const files = await client.beta.files.list({ scope_id: sid, betas: [BETA] } as any);
  for (const f of files.data) {
    console.log(f.id, f.filename);
    if (f.filename.endsWith("bug-report.md")) { const resp = await client.beta.files.download(f.id); writeFileSync("bug-report.md", Buffer.from(await resp.arrayBuffer())); console.log("downloaded bug-report.md"); }
  }
