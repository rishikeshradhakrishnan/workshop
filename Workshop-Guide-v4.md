...
| M4 | [Automation & scale](modules/04-automation-and-scale.md) | Claude Code without a human at the keyboard: `claude -p` with JSON/stream-JSON output, `--json-schema`, locked-down permissions and budgets, session resume, the headless trust caveat; `claude-code-action` for `@claude` mentions and PR review; a guided tour of background sessions, dynamic workflows, Claude Code on the web and Routines with a "when to use what" table. Two lab paths. 30 min. |
